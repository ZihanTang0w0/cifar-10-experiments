...
CACHE_FILE="./cached_ref_stats.npz" # Where to save the pre-computed stats
...
for dataset in "${ALL_DATASETS[@]}"; do
    if [[ "$dataset" == *mode_flow* && "$dataset" == *seed* ]]; then
        DATASETS+=("$dataset")
    fi
done

echo ""
echo "Found ${#DATASETS[@]} Flow datasets to evaluate."
echo "--------------------------------------------------------"
